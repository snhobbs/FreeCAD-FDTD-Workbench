...
from utilsOpenEMS.GlobalFunctions.GlobalFunctions import _r
...
class CommonScriptLinesGenerator:
    ...
    def getItemsByClassName(self):
        categoryCount = (
            self.form.objectAssignmentRightTreeWidget.invisibleRootItem().childCount()
        )
        categoryNodes = [
            self.form.objectAssignmentRightTreeWidget.topLevelItem(k)
            for k in range(categoryCount)
        ]
        itemsByClassName = {}
        for m, categoryNode in enumerate(categoryNodes):  # for each category
            for k in range(
                categoryNode.childCount()
            ):  # for each child item in a given category
                item = categoryNode.child(k)
                itemData = item.data(0, QtCore.Qt.UserRole)
                if not itemData:
                    continue
                itemClassName = itemData.__class__.__name__
                if itemClassName not in itemsByClassName:
                    itemsByClassName[itemClassName] = [[item, itemData]]
                else:
                    itemsByClassName[itemClassName].append([item, itemData])

        print(
            "generateOpenEMSScript: Item classes found = ",
            ", ".join(itemsByClassName.keys()),
        )
    ...
    def createOuputDir(self, outputDir):
        programname = os.path.basename(
            self.cadHelpers.getCurrDocumentFileName()
        )  # FreeCAD filename with extension
        programdir = os.path.dirname(
            self.cadHelpers.getCurrDocumentFileName()
        )  # FreeCAD file directory
        programbase, ext = os.path.splitext(
            programname
        )  # FreeCAD filename without extension
        ...
        #
        if outputDir is None or outputDir is False:
    ...
    def getModelCoordsType(self):
        ...
        if self.form.gridSettingsTreeView.topLevelItemCount() > 0:
            coordsType = (
                self.form.gridSettingsTreeView.topLevelItem(0)
                .data(0, QtCore.Qt.UserRole)
                .coordsType
            )
        else:
