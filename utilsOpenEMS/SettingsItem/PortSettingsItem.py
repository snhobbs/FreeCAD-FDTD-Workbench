...
class PortSettingsItem(SettingsItem):
    def __init__(
        ...
        mslFeedShiftValue="",
        mslFeedShiftUnits="",
        mslMeasPlaneShiftValue="",
        mslMeasPlaneShiftUnits="",
        mslMaterial="",
        mslPropagation="",
        waveguideRectDir="",
        waveguideCircDir="",
        coaxialConductorMaterial="",
        coaxialMaterial="",
        coaxialPropagation="",
        coaxialInnerRadiusValue=0,
        coaxialInnerRadiusUnits="",
        coaxialShellThicknessValue=0,
        coaxialShellThicknessUnits="",
        coaxialFeedpointShiftValue=0,
        coaxialFeedpointShiftUnits="",
        coaxialMeasPlaneShiftValue=0,
        coaxialMeasPlaneShiftUnits="",
        coplanarMaterial="",
        coplanarPropagation="",
        coplanarGapValue=0,
        coplanarGapUnits="",
        coplanarFeedpointShiftValue=0,
        coplanarFeedpointShiftUnits="",
        coplanarMeasPlaneShiftValue=0,
        coplanarMeasPlaneShiftUnits="",
        striplineMaterial="",
        striplinePropagation="",
        striplineHeightValue=0,
        striplineHeightUnits="",
        striplineFeedpointShiftValue=0,
        striplineFeedpointShiftUnits="",
        striplineMeasPlaneShiftValue=0,
        striplineMeasPlaneShiftUnits="",
    ):
        self.name = name
        self.type = type
    ...
    def getMicrostripStartStopCoords(self, bbCoords, sf):
        ...
        elif self.direction.startswith(
            ("YZ plane, right layer", "z-theta, outside layer")
        ):
    ...
    def getCircularWaveguidStartStopRadius(self, bbCoords, sf):
        ...
        if self.direction.startswith("z"):
            waveguideRadius = min(
                abs(portStartX - portStopX), abs(portStartY - portStopY)
            )
        elif self.direction.startswith(("x", "r")):
            waveguideRadius = min(
                abs(portStartY - portStopY), abs(portStartZ - portStopZ)
            )
        elif self.direction.startswith(("y", "theta")):
            waveguideRadius = min(
                abs(portStartX - portStopX), abs(portStartZ - portStopZ)
            )
    ...
    def getCoaxialInnerRadiusShellThicknessFeedShiftMeasShift(self):
        coaxialInnerRadius = self.coaxialInnerRadiusValue * self.getUnitsAsNumber(
            self.coaxialInnerRadiusUnits
        )
        coaxialShellThickness = self.coaxialShellThicknessValue * self.getUnitsAsNumber(
            self.coaxialShellThicknessUnits
        )
        coaxialFeedShift = self.coaxialFeedpointShiftValue * self.getUnitsAsNumber(
            self.coaxialFeedpointShiftUnits
        )
        coaxialMeasPlaneShift = self.coaxialMeasPlaneShiftValue * self.getUnitsAsNumber(
            self.coaxialMeasPlaneShiftUnits
        )

        return (
            coaxialInnerRadius,
            coaxialShellThickness,
            coaxialFeedShift,
            coaxialMeasPlaneShift,
        )

    def getCoplanarStartStopAndGapWidthAndEVecStr(self, currSetting, bbCoords, sf):
        #
        ...
        gapWidth = currSetting.coplanarGapValue * currSetting.getUnitsAsNumber(
            currSetting.coplanarGapUnits
        )
        gapWidth_freeCAD_units = (
            currSetting.coplanarGapValue
            * currSetting.getUnitsAsNumber(currSetting.coplanarGapUnits)
            / self.getFreeCADUnitLength_m()
        )

        if currSetting.direction.startswith(
            ("XY", "r-theta")
        ) and currSetting.coplanarPropagation.startswith(("x", "r")):
            coplanarEVecStr = "[0 1 0]"
            portStartY += gapWidth_freeCAD_units
            portStopY -= gapWidth_freeCAD_units
        elif currSetting.direction.startswith(
            ("XY", "r-theta")
        ) and currSetting.coplanarPropagation.startswith(("y", "theta")):
            ...
            ("XZ", "z-theta")
        ) and currSetting.coplanarPropagation.startswith("z"):
            coplanarEVecStr = "[1 0 0]"
            portStartX += gapWidth_freeCAD_units
            portStopX -= gapWidth_freeCAD_units
        elif currSetting.direction.startswith(
    ...
    def getStriplineStartStopAndHeight(self, bbCoords, sf):
        ...
        if self.striplinePropagation in [
            "x+",
            "y+",
            "r+",
            "theta+",
        ] and self.direction in ["XY plane", "r-theta plane"]:
            portStartX = _r(sf * bbCoords.XMin)
            portStopX = _r(sf * bbCoords.XMax)
            portStartY = _r(sf * bbCoords.YMin)
            portStopY = _r(sf * bbCoords.YMax)
        elif (
            self.striplinePropagation in ["x+", "z+", "r+"]
            and self.direction == "XZ plane"
        ):
            portStartX = _r(sf * bbCoords.XMin)
            portStopX = _r(sf * bbCoords.XMax)
            portStartZ = _r(sf * bbCoords.ZMin)
            ...
            self.striplinePropagation in ["y+", "z+", "theta+"]
            and self.direction == "YZ plane"
        ):
            portStartY = _r(sf * bbCoords.YMin)
            portStopY = _r(sf * bbCoords.YMax)
            portStartZ = _r(sf * bbCoords.ZMin)
            ...
            self.striplinePropagation in ["x-", "y-", "r-", "theta-"]
            and self.direction == "XY plane"
        ):
            portStartX = _r(sf * bbCoords.XMax)
            portStopX = _r(sf * bbCoords.XMin)
            portStartY = _r(sf * bbCoords.YMax)
    ...
    def getCurveStartStop(self, bbCoords, sf):
        if _bool(self.direction) is False:
            portStartX = _r(sf * bbCoords.XMin)
            portStartY = _r(sf * bbCoords.YMin)
            portStartZ = _r(sf * bbCoords.ZMin)
            portStopX = _r(sf * bbCoords.XMax)
