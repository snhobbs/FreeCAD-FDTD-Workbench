...
class GuiHelpers:
    ...
    def initRightColumnTopLevelItems(self):
        ...
        topItem.setIcon(
            0, QtGui.QIcon(os.path.join(self.APP_DIR, "img", "material.svg"))
        )
        ...
        topItem.setIcon(
            0, QtGui.QIcon(os.path.join(self.APP_DIR, "img", "excitation.svg"))
        )
        self.form.objectAssignmentRightTreeWidget.insertTopLevelItem(0, topItem)
        ...
        topItem.setIcon(
            0, QtGui.QIcon(os.path.join(self.APP_DIR, "img", "lumpedpart.svg"))
        )
        self.form.objectAssignmentRightTreeWidget.insertTopLevelItem(0, topItem)
    ...
    def removeAllMeshPriorityItems(self):
        ...
        print(
            "ITEM TO REMOVE: "
            + str(self.form.meshPriorityTreeView.invisibleRootItem().childCount())
        )
        ...
        for k in reversed(range(priorityItemsCount)):
            print(
                "REMOVING ITEM "
                + self.form.meshPriorityTreeView.takeTopLevelItem(k).text(0)
            )
    ...
    def updateMeshPriorityDisableItems(self):
        ...
        for k in range(itemsCount):
            ...
            gridParent = self.form.objectAssignmentRightTreeWidget.findItems(
                itemNameFields[1].strip(), QtCore.Qt.MatchRecursive
            )
            if len(gridParent) > 0:
                if not _bool(
                    gridParent[0].data(0, QtCore.Qt.UserRole).topPriorityLines
                ):
                    self.form.meshPriorityTreeView.topLevelItem(k).setBackground(
                        0, QtGui.QColor("white")
                    )
                else:
                    self.form.meshPriorityTreeView.topLevelItem(k).setBackground(
                        0, QtGui.QColor("lightgray")
                    )
    ...
    def addSettingsItemGui(self, settingsItem):
        ...
        itemTypeReg = re.search(
            "(.*)SettingsItem", str(settingsItem.__class__.__name__)
        )
        typeStr = itemTypeReg.group(1)

        treeItem.setIcon(
            0, QtGui.QIcon(os.path.join(self.APP_DIR, "img", typeStr.lower() + ".svg"))
        )
        ...
        targetGroup = self.form.objectAssignmentRightTreeWidget.findItems(
            typeStr, QtCore.Qt.MatchExactly
        )
    ...
    def removePriorityName(self, priorityName):
        ...
        while priorityItemRemoved:
            ...
            priorityItemsCount = (
                self.form.objectAssignmentPriorityTreeView.topLevelItemCount()
            )
            for k in range(priorityItemsCount):
                priorityItem = self.form.objectAssignmentPriorityTreeView.topLevelItem(
                    k
                )
    ...
    def setComboboxItem(self, controlRef, text, alternativeEquivalentValues=None):
        ...
        if index >= 0:
            ...
            if alternativeEquivalentValues is not None:
                #
                ...
                for alternativeValueTuple in alternativeEquivalentValues:
                    if alternativeValueTuple[0] == text:
                        print(
                            f"WARNING: For {controlRef} instead {text} trying to use alternative equivalent value {alternativeValueTuple[1]}"
                        )
                        self.setComboboxItem(controlRef, alternativeValueTuple[1])
                    elif alternativeValueTuple[1] == text:
                        print(
                            f"WARNING: For {controlRef} instead {text} trying to use alternative equivalent value {alternativeValueTuple[0]}"
                        )
                        self.setComboboxItem(controlRef, alternativeValueTuple[0])
                return
            else:
                print(
                    f"WARNING: Cannot set for {controlRef} item {text}, wasn't found in items."
                )
                return

            print(
                f"WARNING: Cannot set for {controlRef} item {text}, alternative value was not found."
            )
    ...
    def hasPortSomeObjects(self, portName):
        ...
        itemWithSameName = self.form.objectAssignmentRightTreeWidget.findItems(
            portName, QtCore.Qt.MatchFixedString | QtCore.Qt.MatchRecursive
        )
        for item in itemWithSameName:
            ...
            if item.parent().parent() is None and item.parent().text(0) == "Port":
    ...
    def getGridGroupObjectAssignmentTreeItem(self, groupName):
        gridGroupWidgetItems = self.form.objectAssignmentRightTreeWidget.findItems(
            groupName, QtCore.Qt.MatchExactly | QtCore.Qt.MatchFlag.MatchRecursive
        )
        gridGroupItem = None
